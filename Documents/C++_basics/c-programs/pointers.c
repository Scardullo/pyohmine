...
void birthday(int* age){
    // functions in C are pass by value...  needs to be pass by reference
    (*age)++;  // <- by adding parenthesis we are forcing operator precedence,
}              //    without them we would add 1 to the memory address before 
               //    derefrencing, dereferencing is what the "*" is for telling
               //    the function to add to the variable 