...
void deleteStudent() {
    ...
    while (t && t->id != id) {  // <- While t is not NULL AND t->id is not equal to id
        prev = t;
        t = t->next;
    }
    if (!t) {
...
void editStudent() {
    ...
    scanf(" %c", &choice); // leading space " %c" ignores "\n"
    if (choice == 'y' || choice == 'Y') {
