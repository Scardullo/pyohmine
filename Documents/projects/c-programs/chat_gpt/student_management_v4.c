...
void sortStudents(int sortBy) {
    ...
    do {
        ...
        while (ptr->next != lastPtr) {
            ...
            if (sortBy == 1) // sort by name A–Z
                compare = strcmp(ptr->name, ptr->next->name) > 0;
            else if (sortBy == 2) // sort by grade High–Low
                compare = ptr->grade < ptr->next->grade;

            if (compare) {
                // Swap the student data instead of nodes
                int tempId = ptr->id;
                char tempName[50];
