...
screen = display.set_mode(size, DOUBLEBUF)#DOUBLEBUF->#front buffer → what’s currently visible on the screen\n  
screen_rect = screen.get_rect()			  			  #back buffer  → where you draw everything for the next frame
# Sets window icon
display.set_icon(transform.scale(image.load('images/icon.png'), (256,256)))		# Window Icon
display.set_caption('Loading...')
# Transparent surface
alpha_screen = Surface(size, SRCALPHA).convert_alpha()  # used for transparent overlay like\n
alpha_screen.fill((0,0,0,50))							# buttons, stats, screen fading/transitioning, menus, etc
myClock = time.Clock()
FPS = 60
mode = 'menu'
...
def mul_lines(f_path, t, s, wid, f_color=BLACK):
	...
	lines = []
	while f.size(t)[0] > wid: # [0] is width where [1] would be height
		pos = len(t)
		while f.size(t[0:pos])[0] > wid:   # this inner loop will keep iterating through [0:pos]\n
			pos = t.rfind(' ', 0, pos)     # until the space is found that allows left most string that fits in 'wid'\n
			if pos == -1:				   # because .rfind looks for rightmost space (until this inner loop condition is met)
				continue
		lines.append(t[0:pos])	# append [0:pos] (new pos)
		t = t[pos+1:]		# now 't' is the rest of the string [pos+1:]<- (new pos -to-> end of string) then back up to loop again
	lines.append(t) # finally append last string

	totHeight = f.size(lines[0])[1] * len(lines)   # [1] or the 'height' of one line. so * len(lines) gives\n
	surf = Surface((wid, totHeight), SRCALPHA)     #  total height of the surface (total height of all lines[])
	surf.fill((0,0,0,0))
	for p in range(len(lines)):
		lineFont = f.render(lines[p], True, f_color)
		lineFontRect = lineFont.get_rect()
		lineFontRect.midtop = surf.get_width()//2, p * lineFont.get_height() # could also use lineFontRect.get_height()
...
def get_image(sheet, pos):
	...
	img.blit(sheet, img.get_rect(), pos)  #<- dest_surface.blit(source_surface, dest_rect, source_rect) || dest_surface.blit(source_surface, dest_position, area)
	return img

# Gets spritesheet coordinates from .txt file
def get_cos(file_path):
	f = open(file_path).read().strip() # gets rid of trailing \n and whiespaces at begin and end of file
	co_list = f.split('\n')     # splits file line by line
	for pos in range(len(co_list)):
		co_list[pos] = co_list[pos].split(' ') # splits each line by spaces
		for s in range(4):
			co_list[pos][s] = int(co_list[pos][s]) # turns trhe strings into integers

	return co_list

# Makes every pixel on a Surface into a desired color
def color_change(surf, r, g, b, a=255):   # 'a' is 'alpha channel' (transparency) 0 = fully transparent\n
	surf = surf.copy()					  #  255 = fully opaque
	w, h = surf.get_size()
	for x in range(w):
		for y in range(h):
...
def player_scroll():
    ...
    if player.rect.left > width - width//5 or player.rect.right < width//5:
        off_screen = True
        # Player is near left or right edge → we need to scroll the world
        # ⬅️ or ➡️ depending on which side

    # ----------------------------
    # Compute how much to move the world
    # ----------------------------
    if off_screen:
        move_by = (player.rect.centerx - screen_rect.centerx) / 20
        # move_by is positive if player is right of center → scroll world left
        # move_by is negative if player is left of center → scroll world right
        go_ahead = True
        if abs(player.rect.centerx - screen_rect.centerx) < width//8:
            off_screen = False
            # Player roughly centered → stop scrolling
    else:
        if player.rect.left < width // 4:
            move_by = -abs(player.vel.x)
            go_ahead = True
            # Player near left edge
            # move_by negative → world will shift right → player appears to move left
        elif player.rect.right > width - width // 4:
            move_by = abs(player.vel.x)
            go_ahead = True
            # Player near right edge
            # move_by positive → world will shift left → player appears to move right

    # ----------------------------
    # Scroll the world if needed
    # ----------------------------
    if go_ahead:
        rounded_move_by = round(move_by)

        # Move all game sprites (platforms, enemies, items)
        for s in all_sprites:
            s.rect.x -= rounded_move_by
            if s in enemies:
                s.pos[0] -= rounded_move_by
            # ⬅️ World moves opposite to player intent

        # Move background/foreground layers
        ...
        # ⬅️ Parallax and interactive objects scroll with world

        # Update player's true world position (used for collisions, physics)
        player.pos.x -= move_by
        # Player rect.x may stay mostly the same, but pos.x keeps world coordinates correct

        # Move background slower for parallax effect
        background_rect.x -= rounded_move_by / 2
        # ⬅️ slower than foreground → depth effect

        # Update spawn point so respawns work with scrolled world
...
def level_creator(pic,player_type,background=0):
	...
	for x in range(p_width):
		for y in range(p_height):
			...
			# E.g. an alpha value of 254 means to move the object down one block from where the actual pixel is
	...
	for bridge in bridges:
		...
		for bb in back_blocks:
			if bb.rect.colliderect(bridge.rect) and bb.touching and not found: # touching =  back blocks revealed when gone behind
				back_front_sprites.add(bridge)								   # found =  is flag set so once its decided which group to add to\n
				found = True												   # those blocks are not accidentaly added to another group
		for w in waters:
...
def load_images():
	...
	title_text_render = mul_lines('fonts/GameBoy.ttf', 'Alien Adventures', 36, font.Font('fonts/GameBoy.ttf', 36).size('Adventures')[0]+10) # +10 is padding
...
def scroll(d, background=False, player=False):
	...
	for i in range(91):
		...
		event.pump()  # It does not return any events—it just updates the queue.
		mx, my = mouse.get_pos()
		display.flip()
		time.wait(3)
	let_go = False
...
def level_complete(screen_copy, title='Level Complete!', transition_time=60, main_background_color=LIGHT_BROWN, title_background_color=DARK_BROWN):  # transition_time may be left over(not used)
	...
	for i in range(0, 121, 5):		# <-window that scales up smoothly from nothing to full size, while the screen darkens behind it
		a += 5
		screen.fill(WHITE)
		alpha_screen.fill((0,0,0,a))	# Darkens surroundings
		screen.blit(screen_copy, (0,0))		# What is behind the popup...

		coeff = popup_rect.width / sin(radians(121))							# coeff is so when (i) reaches 121 new_x will be size of popup
		new_x = coeff * sin(radians(i))											# otherwise it would be 86%. sin(radians(121)) will increase popup
		new_y = coeff*popup_rect.height/popup_rect.width * sin(radians(i))		# to slightly larger then back dow to final size
	...
	while outcome == None:	# Keeps running until a button is clicked.
		...
		mx, my = mouse.get_pos()
		screen.fill(WHITE)
		alpha_screen.fill((0,0,0,a))  #<- 'a' is still ubdated from for loop above
...
# Similar to the level_complete, but more customizable: Custom button text, custom title, custom header text... # transition_time may be left over(not used)
def popup(title, header_txt, button_text, screen_copy, transition_time=60, main_background_color=LIGHT_BROWN, title_background_color=DARK_BROWN, main_button_color=(0,200,0), pics=[]):
	...
	option_buttons = []
	step = popup_rect.width / (len(button_text)+1) # <- | - step - [button] - step - [button] - step - | 
	for n,t in enumerate(button_text):
		tmp = Button(t, [0,0])
		if n == 0:
			tmp.hover_color = main_button_color  # first button different color than others
	...
	if pop_out:
		for i in range(120, -1, -5):		# Transition for the popup to disappear
			a -= 5
			screen.fill(WHITE)
			alpha_screen.fill((0,0,0,a))
			screen.blit(screen_copy, (0,0))		# What is behind the popup...
			...
			new_x = coeff * sin(radians(i))											# Black magic
			new_y = coeff*popup_rect.height/popup_rect.width * sin(radians(i))		#
			scaled_popup_surf = transform.scale(popup_surf, (round(new_x), round(new_y)))	# shrink popup until gone
			scaled_popup_rect = scaled_popup_surf.get_rect()
			scaled_popup_rect.center = width//2, height//2			# Align the scaling popup to center
			alpha_screen.blit(scaled_popup_surf, scaled_popup_rect)
...
class Button:
	...
	def make_image(self, new_button_col, new_text_col):
		...
		self.rect = self.image.get_rect()
		tmp = Rect(self.rect.x+self.rad//2, self.rect.y+self.rad//2, self.rect.width-self.rad, self.rect.height-self.rad)  # smaller rect inside of image rect
...
for i in range(9):
	buttons.append(Button("Level " + str(i+1), [width + 64 + 960/3*(i%3), height + 192 + (i//3)*128], dim=[64*3, 64]))  # widht -> i%3 -> 012, 012, 012  |   height -> i//3 -> 000, 111, 222
...
class Player(sprite.Sprite):
			...
			for hits in h:
				if not kp[K_s]:
					if self.vel.y > 7*player.size:
						if hits.rect.colliderect(self.rect):  # checks just self.rect because falling fast
							self.pos.y = hits.rect.top
							self.vel.y = 0
							self.acc.y = 0
							self.jumping = False
					elif self.vel.y > 0 and (hits.rect.collidepoint(self.rect.midbottom) or hits.rect.collidepoint(self.rect.bottomright) or hits.rect.collidepoint(self.rect.bottomleft)):  # checks specific points because slower
						self.pos.y = hits.rect.top
						self.vel.y = 0
						self.acc.y = 0
						self.jumping = False
	...
	# Finds the farthest checkpoint. If there are none, brings player back to spawn point
	...
	def injure(self):
		...
		if self.hurt_counter % 8 == 0:			#
			self.image_on = 1 - self.image_on	# if current = 1:  1 - 1 = 0 (OFF) | if current = 1: 1 - 0 = 1 (ON) 
...
class JumpPad(sprite.Sprite):
	def __init__(self, pos):
		...
		self.images = [get_image(misc_sheet2,misc_cos2[7]),get_image(misc_sheet2,misc_cos2[6]),get_image(misc_sheet2,misc_cos2[5])] # <- self.images = [img7, img6, img5]
		self.images.append(self.images[1])  # <- self.images = [img7, img6, img5, img6]
		self.images.append(self.images[0])  # <- self.images = [img7, img6, img5, img6, img7]
