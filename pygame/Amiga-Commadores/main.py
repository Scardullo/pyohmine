...
def load_images():
	...
	title_text_render = mul_lines('fonts/GameBoy.ttf', 'Amiga Commadores', 36, font.Font('fonts/GameBoy.ttf', 36).size('Commadores')[0]+10)
...
def pause(s_copy):
	...
	for i in range(width, width - side_bar_rect.width-1, -animation_speed):
		...
		screen_copy = screen.copy()    # the scene after the pause menu has finished sliding in
		display.flip()				   # so it can be used for future popups instaed of fully opaque s_copy
	...
	while result == None:
		...
		display.flip()
		myClock.tick()
		display.set_caption('Amiga Commadores')
...
def read_data():
	global music_bool, current_theme, levels_unlocked, all_gems_collected # function will modify these global variables instead of creating local copies
